...
// ================== BUTTON HELPERS ==================
function findAndClickActionButton(buttonTextPattern, timeout = 30000) {
  // Wait for UI to stabilize completely
  waitForLivewireIdle();
  cy.wait(3000);
  ...
  cy.get('button, [role="button"]').then(($buttons) => {
    ...
    cy.log('Available buttons:', buttonTexts.filter(text => text.length > 0));
  });
  
  // Try multiple approaches to find and click the button
  cy.get('body').then(($body) => {
    // Approach 1: Look for button in common containers
    const containers = $body.find('.modal-footer, .form-actions, .actions-container, .submit-section, .je-footer, .footer-actions, .action-buttons');
    let found = false;
    
    containers.each((i, container) => {
      const buttons = Cypress.$(container).find('button, [role="button"]');
      buttons.each((j, btn) => {
        const btnText = Cypress.$(btn).text().trim();
        if (buttonTextPattern.test(btnText)) {
          if (Cypress.$(btn).is(':visible') && !Cypress.$(btn).is(':disabled')) {
            cy.wrap(btn)
              .scrollIntoView()
              .click({ force: true });
            found = true;
    ...
    // Approach 2: Search entire body for buttons
    ...
    // Approach 3: Look for primary action buttons
    const primaryButtons = $body.find('.btn-primary, .bg-primary, .bg-blue, .bg-green, [class*="primary"]');
    primaryButtons.each((i, btn) => {
      const btnText = Cypress.$(btn).text().trim();
      if (btnText.length > 0 && Cypress.$(btn).is(':visible') && !Cypress.$(btn).is(':disabled')) {
        cy.wrap(btn)
          .scrollIntoView()
          .click({ force: true });
        found = true;
    ...
    // Final fallback: use the original approach
    cy.contains("button, a, [role='button']", buttonTextPattern, { timeout })
      .scrollIntoView()
      .should("be.visible")
      .and("not.be.disabled")
      .click({ force: true });
...
function verifySuccessMessage() {
  cy.contains(/submitted successfully|approved successfully|Journal Entry (submitted|approved)|success|completed|approved/i, { timeout: 20000 })
    .should("be.visible");
}

// ================== APPROVAL SPECIFIC FUNCTIONS ==================
function navigateToSubmittedJEs() {
  // Navigate to the journal entries list that shows submitted entries
  cy.visit(`${BASE}/journal-entries?tenant=${TENANT_ID}&status=submitted`, {
    failOnStatusCode: false,
  });
  
  // Alternatively, try different status filters
  cy.visit(`${BASE}/journal-entries?tenant=${TENANT_ID}&status=pending`, {
    failOnStatusCode: false,
  });
...
function findAndOpenJEForApproval() {
  // Look for the most recent submitted JE
  cy.get("table:visible, .table:visible, [role='grid']:visible", { timeout: 20000 })
    .should("be.visible")
    .within(() => {
      // Look for rows with status "Submitted" or "Pending"
      cy.contains('tr', /submitted|pending/i, { timeout: 15000 })
        .first()
        .within(() => {
          // Click on the row or action button to open details
          cy.get('button:contains("View"), button:contains("Details"), [role="button"]')
            .first()
            .click({ force: true });
        });
    });
...
// ================== SPEC ==================
describe("JE auto-fill account + debit/credit -> add -> grid row appears -> submit -> approve", () => {
  it("adds items, submits and approves the JE", () => {
    ...
    cy.log("Submitting journal entry");
    findAndClickActionButton(/^(Submit|Approve|Post)$/i);
    verifySuccessMessage();

    // ================== APPROVE JOURNAL ENTRY ==================
    cy.log("Approving journal entry");
    
    // Wait a bit after submission
    waitForLivewireIdle();
    cy.wait(5000);
    
    // Navigate to submitted JEs
    navigateToSubmittedJEs();
    
    // Find and open the submitted JE
    findAndOpenJEForApproval();
    
    // Wait for approval page to load
    waitForLivewireIdle();
    cy.wait(3000);
    
    // Click the approve button
    cy.log("Clicking approve button");
    findAndClickActionButton(/^(Approve|Confirm|Accept|Approve Entry)$/i);
    
    // Verify approval success
    cy.contains(/approved successfully|Journal Entry approved|successfully approved|approval complete/i, { timeout: 20000 })
      .should("be.visible");
    
    // Additional verification that the JE status changed
    cy.contains(/status.*approved|approved.*status/i, { timeout: 15000 })
      .should("be.visible");
  });
});